<script>
    ...
    onMount(() => {
        ...
        svg = d3
            .select("#bullet_chart_clutch_raw")
            .selectAll("svg")
            .data(data)
            .enter()
    ...
    const update = (msg) => {
        if (bitsMapNumbers.clutchBits && calibrationMapNumbers.clutchCalibration) {
            ...
            const measures1 = (width / +bitsMapNumbers.clutchBits[0]) * msg.clutch.raw
    ...
    const updateGraph = () => {
        if (bitsMapNumbers.clutchBits && calibrationMapNumbers.clutchCalibration) {
            ...
            const markers1 = (width / +bitsMapNumbers.clutchBits[0]) * +calibrationMapNumbers.clutchCalibration[1];
            select.selectAll(".marker.s1").attr("x1", markers1).attr("x2", markers1)

            const markers2 = (width / +bitsMapNumbers.clutchBits[0]) * +calibrationMapNumbers.clutchCalibration[2];
            select.selectAll(".marker.s2").attr("x1", markers2).attr("x2", markers2)

            const markers3 = (width / +bitsMapNumbers.clutchBits[0]) * +calibrationMapNumbers.clutchCalibration[3];
