<script>
    ...
    const update = (msg) => {
        ...
        if ($bitsMap && $bitsMap.clutchBits && $calibrationMap && $calibrationMap.clutchCalibration) {
            ...
            const measures1 = (width / +$bitsMap.clutchBits[0]) * msg.clutch.raw
    ...
    $: {
        if ($bitsMap && $bitsMap.clutchBits && $calibrationMap && $calibrationMap.clutchCalibration) {
            ...
            const markers1 = (width / +$bitsMap.clutchBits[0]) * +$calibrationMap.clutchCalibration[1];
            select.selectAll(".marker.s1").attr("x1", markers1).attr("x2", markers1)

            const markers2 = (width / +$bitsMap.clutchBits[0]) * +$calibrationMap.clutchCalibration[2];
            select.selectAll(".marker.s2").attr("x1", markers2).attr("x2", markers2)

            const markers3 = (width / +$bitsMap.clutchBits[0]) * +$calibrationMap.clutchCalibration[3];
